export const SEARCH_CHARACTERS = "SEARCH_CHARACTERS";
export const SEARCH_EPISODES = "SEARCH_EPISODES";
export const SEARCH_LOCATIONS = "SEARCH_LOCATIONS";
export const GET_CHARACTER = "GET_CHARACTER";
export const GET_EPISODE = "SEARCH_EPISODE";
export const GET_LOCATION = "SEARCH_LOCATION";
export const SET_LOADER = "SET_LOADER";
export const SET_LOADER2 = "SET_LOADER2";
export const SET_ERROR = "SET_ERROR";
export const GET_CHARACTER_EPISODES = "SET_CHARACTER_EPISODES";
export const GET_EPISODE_CHARACTERS = "SET_EPISODE_CHARACTERS";
export const GET_LOCATION_RESIDENTS = "SET_LOCATION_RESIDENTS";
export const CLEAR_CHARACTERS = "CLEAR_CHARACTERS";
export const CLEAR_EPISODES = "CLEAR_EPISODES";
export const CLEAR_LOCATIONS = "CLEAR_CHARACTERS";
export const CLEAR_ERROR = "CLEAR_ERROR";
